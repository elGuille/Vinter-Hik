...
class TargetVC: UIViewController, UIGestureRecognizerDelegate {
   ...
   override func viewDidLoad() { super.viewDidLoad()
      ...
      navigationDisplayⓞ = NavigationDisplay(displayViewⓞ: view, maxDepth＃: maxVinterDepth＃, rootLabelⓞ: Ψ﹫, delegateⓞ: self, tapHandlerƒ: #selector(TargetVC.navigateInTree))
...
// MARK: AVPlayer Delegate
   ...
   func videoFinishedPlaying(notification: NSNotification) { status🅔 = .Stopped ; videoAtEndⓑ = NO }
...
//      var imagesⓞ🅐 = [UIImage]() ; for imageIndex＃ in 0...imagesPerTarget＃ { imagesⓞ🅐.append(UIImage(named: "\(imageIndex＃).png")!) }
      ...
      newTargetⓞ.setFrame(widthⓖ: screenShortestDimensionⓖ * targetSizeFactorⓖ, heightⓖ: screenShortestDimensionⓖ * targetSizeFactorⓖ)
...
//      let panPointⓖ² = sender.locationInView(view)
   ...
   func navigateInTree(sender: NavigationTapGestureRecognizer) {
      print("Navigating to \(sender.depthLevel＃)")
      navigationDisplayⓞ.natigateTo(sender.depthLevel＃)
   }
