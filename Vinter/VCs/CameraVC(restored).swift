...
class CameraVC: UIViewController, AVCaptureFileOutputRecordingDelegate {
   ...
   func startREC() {
      recCircleIV﹫.hidden = false
      recRecIV﹫.hidden = false
      finishButton﹫.hidden = true
      startRecThrob()
   }
   
   func stopREC() {
      recCircleIV﹫.hidden = true
      recRecIV﹫.hidden = true
   ...
   func sendDelayedStop(delay: NSTimeInterval) {
      HKThreading＋.LAUNCH_THREAD_MAIN_Q(delayInSeconds＃: delay, bodyƒ: {
         ...
         self.stopRecThrob()
         self.stopREC()
      }, completionƒ: { self.stopRequestedⓑ = false })
   }
   ...
   //      presentViewController(activityViewController, animated: true, completion: { self.setButtonsToRecMode() })
   //   }
